	<?php
		if (isset($_REQUEST['edit'])) {
			...
			if (move_uploaded_file($tmp, $path)) {
				$query = "SELECT * FROM surat_masuk WHERE id_surat='$_GET[id]'";
			...
			//jika mengubah file		
				$query 	= "UPDATE surat_masuk SET no_agenda='$no_agenda', no_surat='$no_surat', jenis_surat='$jenis_surat', tanggal_surat='$tanggal_surat', asal_surat='$asal_surat', isi_ringkas='$isi_ringkas', file='$file' WHERE id_surat='$_GET[id]'";
				$sql   	= mysqli_query($connect, $query);
				if ($sql) {
			...
			//jika tidak mengubah file
				$query 	= "UPDATE surat_masuk SET no_agenda='$no_agenda', no_surat='$no_surat', jenis_surat='$jenis_surat', tanggal_surat='$tanggal_surat', asal_surat='$asal_surat', isi_ringkas='$isi_ringkas' WHERE id='$_GET[id]'";
				$sql	= mysqli_query($connect, $query);
				if ($sql) {
					echo '<script language="javascript">
						  window.alert("Data berhasil di ubah")
