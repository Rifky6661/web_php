	<?php 
		if (isset($_REQUEST['edit'])) {
			...
			if (move_uploaded_file($tmp, $path)) {
				$query 	= "SELECT * FROM surat_keluar WHERE id_surat='$_GET[id]'";
			...
			//jika mengubah file
				$query 	= "UPDATE surat_keluar SET no_agenda='$no_agenda', no_surat='$no_surat', jenis_surat='$jenis_surat', tanggal_kirim='$tanggal_kirim', tujuan='$tujuan', isi_ringkas='$isi_ringkas', file='$file' WHERE id_surat='$_GET[id]'";
				$sql	= mysqli_query($connect, $query);
				if ($sql) {
					echo '<script> 
			...
			//jika tidak mengubah file
				$query 	="UPDATE surat_keluar SET no_agenda='$no_agenda', no_surat='$no_surat', jenis_surat='$jenis_surat', tanggal_kirim='$tanggal_kirim', tujuan='$tujuan', isi_ringkas='$isi_ringkas' WHERE id_surat='$_GET[id]'";
				$sql	= mysqli_query($connect, $query);
				if($sql){
					echo '<script>
							window.alert("Data berhasil di ubah");
